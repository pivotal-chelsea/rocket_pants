module RocketPants
  module Respondable
    ...
    def self.pagination_type(object)
      if object.respond_to?(:num_pages) && object.respond_to?(:current_page)
        :kaminari
      elsif defined?(WillPaginate::Collection) && (object.is_a?(WillPaginate::Collection) || object.is_a?(ActiveRecord::Relation))
        :will_paginate
      else
        nil
      end
